...
function FirmwareFlasherPanel({
  ...
  const localFlashMethods = useMemo(() => {
    ...
    if (targetType === TargetType.Receiver) {
      methods.push({ value: FlashMethod.APPassthru, label: 'AP Passthru' });
    }
    return methods;
  }, [targetType]);
  ...
  const renderLocalFileUI = () => (
    <div className="form-grid">
      ...
      {/* device selectors */}
      {(flashMethod === FlashMethod.UART || flashMethod === FlashMethod.ESPTool || flashMethod === FlashMethod.APPassthru) &&
        renderComPortRow(
