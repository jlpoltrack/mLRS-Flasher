...
function FirmwareFlasherPanel({
  ...
  // Enforce allowed methods for R9 Rx
  useEffect(() => {
    if (isR9Rx) {
      if (flashMethod !== FlashMethod.STLink && flashMethod !== FlashMethod.APPassthru) {
        setFlashMethod(FlashMethod.STLink);
      }
      // If selected file is not .hex (and we have files) AND method is STLink, try to select a .hex file
      if (flashMethod === FlashMethod.STLink && firmwareFiles.length > 0 && (!selectedFile || !selectedFile.toLowerCase().endsWith('.hex'))) {
  ...
  return (
    <div className="panel">
      ...
      <div className="form-grid">
          ...
          <div className="select-wrapper">
            ...
            >
              {isLoadingFiles ? (
                ...
                firmwareFiles
                  .filter(f => !isR9Rx || (flashMethod === FlashMethod.STLink ? f.filename.toLowerCase().endsWith('.hex') : true))
                  .map(file => (
        ...
        {/* ELRS Bootloader card for R9 Rx - moved above Flash Method */}
        {isR9Rx && firmwareFiles.some(f => f.filename.toLowerCase().endsWith('.elrs')) && (
          <div className="form-group full-width" style={{ marginBottom: '16px' }}>
            <div className="external-flash-card" style={{ marginTop: 0 }}>
              <div className="flash-card-header" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <a 
                  href={firmwareFiles.find(f => f.filename.toLowerCase().endsWith('.elrs'))?.url} 
                  download={firmwareFiles.find(f => f.filename.toLowerCase().endsWith('.elrs'))?.filename}
        ...
        {/* COM port selection */}
        {/* FIX: Now shown for appassthru as well, but hidden for USB-based methods */}
              ...
              {selectedStlink ? (
                ...
                <>
                  <div className="select-wrapper">
                    <select 
                      value="" 
                      onChange={() => {}} // Selection logic usually handled by requestDevice flow
      ...
      {isFrSkyR9 && flashMethod !== FlashMethod.STLink && (
